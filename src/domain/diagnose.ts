 ...
 *
 * The two §4.4 rates themselves live in `./rates` (#14), where backfill's recovery is
 * what asserts them; Rules 1 and 4 only read `floorCompletionRate` from there.
 */
...
function isEngaged(day: ClassifiedDay): boolean {
  return isFloorMet(day.state) || day.state === 'partial' || isReasonedSkip(day);
}
...
function ruleStagnation(habit: Habit, entries: HabitEntry[], asOfDate: string): DiagnosisFlag[] {
  ...
  for (let block = 0; block < weeks; block += 1) {
    ...
    if (aboveFloor > 0 || !inBlock.some((day) => isFloorMet(day.state))) return [];
  }
